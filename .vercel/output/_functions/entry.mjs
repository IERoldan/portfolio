import { renderers } from './renderers.mjs';
import { c as createExports, s as serverEntrypointModule } from './chunks/_@astrojs-ssr-adapter_-GqdiNWw.mjs';
import { manifest } from './manifest_BjctmEdB.mjs';

const serverIslandMap = new Map();;

const _page0 = () => import('./pages/_image.astro.mjs');
const _page1 = () => import('./pages/api/chat.astro.mjs');
const _page2 = () => import('./pages/_lang_/blog/_slug_.astro.mjs');
const _page3 = () => import('./pages/_lang_/blog.astro.mjs');
const _page4 = () => import('./pages/_lang_/faq.astro.mjs');
const _page5 = () => import('./pages/_lang_/privacy.astro.mjs');
const _page6 = () => import('./pages/_lang_/terms.astro.mjs');
const _page7 = () => import('./pages/_lang_.astro.mjs');
const _page8 = () => import('./pages/index.astro.mjs');
const pageMap = new Map([
    ["node_modules/astro/dist/assets/endpoint/generic.js", _page0],
    ["src/pages/api/chat.ts", _page1],
    ["src/pages/[lang]/blog/[slug].astro", _page2],
    ["src/pages/[lang]/blog/index.astro", _page3],
    ["src/pages/[lang]/faq.astro", _page4],
    ["src/pages/[lang]/privacy.astro", _page5],
    ["src/pages/[lang]/terms.astro", _page6],
    ["src/pages/[lang]/index.astro", _page7],
    ["src/pages/index.astro", _page8]
]);

const _manifest = Object.assign(manifest, {
    pageMap,
    serverIslandMap,
    renderers,
    actions: () => import('./noop-entrypoint.mjs'),
    middleware: () => import('./_noop-middleware.mjs')
});
const _args = {
    "middlewareSecret": "b207dafb-d0a6-42f3-9092-6fe594f189fe",
    "skewProtection": false
};
const _exports = createExports(_manifest, _args);
const __astrojsSsrVirtualEntry = _exports.default;
const _start = 'start';
if (Object.prototype.hasOwnProperty.call(serverEntrypointModule, _start)) ;

export { __astrojsSsrVirtualEntry as default, pageMap };
